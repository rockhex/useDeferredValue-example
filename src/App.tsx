  ...
  return (
    <div className="container u-full-width">
      ...
      >
        ...
        <label htmlFor="searchText">Search for player or team</label>
        <input type="text" placeholder="Search..." id="searchText" onChange={onSearchTextChange} value={searchText} />
      </div>
      {isLoading ? (
        <p style={{ textAlign: "center" }}>Loading data...</p>
      ) : (
        <PlayerList players={dataSorted} searchText={searchTextDeffered} teams={data?.teams ?? []} selectedTeam={""} />
      )}
    </div>
  );
};
